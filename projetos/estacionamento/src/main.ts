...
class EstacionamentoFront {
  constructor( private $: (q: string) => HTMLInputElement, private estacionamento = new Estacionamento()) {}
  ...
  encerrar(cells: HTMLCollection) {
    if (cells[2] instanceof HTMLElement) {
      const veiculo = {
        ...
        tempo: new Date().valueOf() - new Date(cells[2].dataset.time as string).valueOf(),
      };
